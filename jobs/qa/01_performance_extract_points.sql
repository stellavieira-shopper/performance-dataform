...
c_lote AS (
  ...
  FROM (
    SELECT *,
      COALESCE(
        TIMESTAMP(SAFE.PARSE_DATETIME('%Y-%m-%dT%H:%M:%S', end_timestamp)),
        TIMESTAMP(SAFE.PARSE_DATETIME('%Y-%m-%d %H:%M:%S', end_timestamp)),
...
c_fresh_picking AS (
  SELECT SAFE_CAST(fp.cod_matricula AS INT64), CAST(fp.user_name AS STRING), CAST(fp.activity_start AS DATETIME), CAST(fp.activity_end AS DATETIME), 'FRESH_PICKING_TABLE', CAST(fp.metric_type AS STRING), CAST(fp.qty AS FLOAT64), CAST(fp.cod_pedido AS STRING), CAST(NULL AS INT64), CAST(NULL AS BOOL), CAST(NULL AS INT64), CAST(fp.metric_description AS STRING), CAST(NULL AS STRING), CAST(NULL AS STRING), CAST(NULL AS STRING), CAST(NULL AS BOOL), CAST(NULL AS STRING), CAST(NULL AS STRING), CAST(NULL AS FLOAT64), CAST(fp.points AS FLOAT64), TIMESTAMP_DIFF(CAST(fp.activity_end AS TIMESTAMP),CAST(fp.activity_start AS TIMESTAMP),SECOND)/3600.0, CAST(fp.reference_date AS DATE), CAST(NULL AS STRING), CAST([] AS ARRAY<INT64>)
  FROM `shopper-datalakehouse-qa.Ranking_Performance.curated_fresh_picking` AS fp
  WHERE fp.cod_matricula IS NOT NULL
    AND NOT EXISTS (
      SELECT 1 FROM calc AS c
      WHERE c.order_code = fp.cod_pedido
        AND c.metric_description = 'ITENS PICKADOS EM FRESH'
    )
),
...
eventos_unificados AS (
  ...
  UNION ALL SELECT * FROM c_fresh_picking
),
